...
export default function CategoryView(props) {
  ...
  function GetCategorySplodos() {
    fetch(`${Config.SERVERURI}/api/getCatSplodos?` + "catId=" + from, {
      credentials: "include",
    }).then((response) =>
  ...
  function onCatNameChangeSubmit() {
    ...
    if (catNameChangeRef.current.value.length >= 1) {
      (async () => {
        const rawResponse = await fetch(`${Config.SERVERURI}/api/catNameChange`, {
          method: "POST",
          headers: {
  ...
  function onCatDelete() {
    (async () => {
      const rawResponse = await fetch(`${Config.SERVERURI}/api/deleteCat`, {
        method: "POST",
        headers: {
          Accept: "application/json",
          "Content-Type": "application/json",
    ...
    <div className="CategoryView">
      <div className="categoryInformation">
        <div className="categoryName">
          <h1 onClick={onCatNameChangePress}>
            {catTitle}📁
            <input
              ref={catNameChangeRef}
              style={catNameChangeStyle}
              className="categoryNameChange"
            ></input>
            <button
              onClick={onCatNameChangeSubmit}
              style={catNameChangeStyle}
              className="changeCatNameBtn"
            >
              SAVE
            </button>
          </h1>
      ...
      <div className="browseArea">
        <div className="splodoWrapper">{splodoRender}</div>
        <div className="btnArea">
          <button className="deleteCatBtn" onClick={onCatDelete}>
            Delete Directory
          </button>
          <button className="deleteCatBtn">
            <Link to="/new" state={{ fromCat: from, fromCatName: catTitle }}>
              Add Splodo
            </Link>
          </button>
        </div>
